...
def get_appointments_by_date(date: str):
    ...
    matches = list(collection.find({
        "$or": [
            {"type.appointments": {"$exists": True}},
            {"type.tasks": {"$exists": True}}
        ],
        "$or": [
            {"type.appointments.appointment1.StartDate": date},
            {"type.appointments.appointment2.StartDate": date},
            {"type.tasks.task1.StartDate": date},
            {"type.tasks.task2.StartDate": date}
        ]
    }))
    for apt in matches:
        apt["_id"] = str(apt["_id"])
